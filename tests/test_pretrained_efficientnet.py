...
from textile_classification.models import PretrainedEfficientNet

class TestPretrainedEfficientNet(unittest.TestCase):
    
    def test_change_fc_layer(self):
        """
        In the function "change_fc_layer", 
        we change the only fc layer using self.model.classifier = nn.Sequential(), so
    ...
    def test_find_num_feaures_extraced_by_cnn_layers(self):
        """
        The original pre-trained model has 1280 features as the input to its FC layer.
        We nee to check if we can get the same number using find_num_feaures_extraced_by_cnn_layers.
        """
        model = PretrainedEfficientNet()
        num_features_grnd_truth = model.model.classifier[1].in_features
        # I found that it dosn't matter what is the input size.
        # The image will always resize to (384, 384)
    ...
    def test_number_of_outputs(self):
        ...
        model = PretrainedEfficientNet(new_in_fearues_of_fc=num_last_hidden_features, 
