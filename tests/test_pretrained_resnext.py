...
from textile_classification.models import PretrainedResNeXt

class TestPretrainedResNeXt(unittest.TestCase):

    def test_change_fc_layer(self):
        """
        In the function "change_fc_layer", 
        we change the only fc layer using self.model.fc = nn.Linear(), so
        we need to check if there is only one fc layer in the pretrained model, and
        to check that if its name is "fc" for having access to it. 
        """
        ...
        self.assertEqual(fc_layer_name, "fc")
    
    def test_find_num_feaures_extraced_by_cnn_layers(self):
        """
        The original pre-trained model has 2048 features as the input to its FC layer.
        We nee to check if we can get the same number using find_num_feaures_extraced_by_cnn_layers.
        """
        model = PretrainedResNeXt()
        num_features_grnd_truth = model.model.fc.in_features
        # I found that it dosn't matter what is the input size.
        # The image will always resize to (232, 232)
        x = torch.randn([1, 3, 232, 232])
    ...
    def test_number_of_outputs(self):
        ...
        model = PretrainedResNeXt(new_in_fearues_of_fc=num_last_hidden_features, 
